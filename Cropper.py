...
class Cropper:
    ...
    def setup(self):
        ...
        if self.landmarks_type == "dlib":
            self.landmark_finder = dlib.shape_predictor("./data/weights/shape_predictor_68_face_landmarks_GTX.dat")
        else:
            raise NotImplementedError

        if self.recognizer_type == "dlib":
            self.recognizer = dlib.face_recognition_model_v1('./data/weights/dlib_face_recognition_resnet_model_v1.dat')
        else:
            raise NotImplementedError
